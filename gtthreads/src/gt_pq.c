...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	//current kcontext
	kthread_context_t *curr_k_ctx = kthread_cpu_map[kthread_apic_id()]; 
	int curr_cpu_id = curr_k_ctx->cpuid;
	kthread_runqueue_t *other_kthread_runq;
	runqueue_t *other_runq;
	...
	//i am an idle kthread
	if(!(runq->uthread_mask))
	{	
		// printf("I am an idle kthread %d\n", curr_cpu_id);
		
		//load balance and if all kernel threads are initialised
		...
		{	
			...
			int num_cpus = 4;
			//skip myself
			for(unsigned i=0; i!=curr_k_ctx->cpuid && i<num_cpus;i++)
			{	
				
				other_kthread_runq =  &(kthread_cpu_map[i]->krunqueue);

				//check this kthread's runq
				gt_spin_lock(&(other_kthread_runq->kthread_runqlock));
				other_runq = other_kthread_runq->active_runq;

				if(other_runq->uthread_mask)
				{
					
					// printf("kthread %d has work \n", curr_cpu_id);

					/* Find the highest priority bucket */
					uprio = LOWEST_BIT_SET(other_runq->uthread_mask);
					prioq = &(other_runq->prio_array[uprio]);
					...
					__rem_from_runqueue(other_runq, u_obj);

					// printf("Load Balanced from cpu %d to cpu %d \n", curr_cpu_id, i);

					gt_spin_unlock(&(other_kthread_runq->kthread_runqlock));
