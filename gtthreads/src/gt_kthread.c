...
extern void gtthread_app_init(int scheduling_mode, bool load_balancing)
{
	kthread_context_t *k_ctx, *k_ctx_main;
	kthread_t k_tid;
	unsigned int num_cpus, inx;

	// scheduling mechanism and load balancing set
	/* Set the scheduler type in kthread shared info */
	ksched_shared_info_t *ksched_info = &ksched_shared_info;
	gt_spin_lock(&(ksched_info->ksched_lock));
	ksched_info->sched_mode = scheduling_mode;
	ksched_info->load_balance = load_balancing;
	gt_spin_unlock(&(ksched_info->ksched_lock));
...
yield_again:
		...
		{
			kthreads_initialized = 0;
			goto yield_again;
		}
		else
		{
			// gt_spin_lock(&(ksched_info->ksched_lock));
			// ksched_info->all_kthreads_init = true;
			// gt_spin_unlock(&(ksched_info->ksched_lock));
			kthreads_initialized = 1;
